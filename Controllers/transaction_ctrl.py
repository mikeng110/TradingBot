...
class TransactionCtrl:
    ...
    def add_to_pending_list(self, item):
        ...
        self.model.data_writer_handler.update_transaction(transaction=item)
    ...
    def load_transactions(self):
        self.transaction_table = Transactions(self.model.db_tradingbot)
        self.asset_info_db = AssetInfoDB(self.model.db_exchange)

        pending = self.transaction_table.get_all_transactions("Pending")
        active = self.transaction_table.get_all_transactions("Active")

        for p in pending:
            t_item = TransactionItem.gen_from_dict(p)
            d_data = self.asset_info_db.fetch_item("Binance", t_item.base_currency, t_item.target_currency)
    ...
    def add_transaction(self, transaction):
        ...
        self.model.data_writer_handler.update_transaction(transaction=transaction)
