...
class TradeableAsset:
    ...
    def insert_asset(self, exchange, base_currency, target_currencies):
        self.c.execute("INSERT INTO Assets (exchange, base_currency, target_currencies) VALUES (?, ?, ?)",
                       (exchange, base_currency, target_currencies))
        self.connection.commit()

    def init_binance(self): #Add everyone when working
        self.c.execute("DELETE FROM Assets") #bad implementation, change to update instead
    ...
    def read_from_db(self):
        self.c.execute('Select * From Assets')
        for row in self.c.fetchall():
            print(row)